...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    l(`${pre} Request parameters - finalPath: ${finalPath}, s3Url: ${s3Url}, service: ${transcriptServices}`)
    ...
    if (!finalPath || !transcriptServices) {
      err(`${pre} Missing required parameters: finalPath or transcriptServices`)
