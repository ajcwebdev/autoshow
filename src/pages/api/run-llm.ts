...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (typeof newRecord.id === 'number') {
      console.log(`[api/run-llm] Verifying database insertion by querying record ID: ${newRecord.id}`)
      verifiedRecord = await dbService.getShowNote(newRecord.id)
      console.log(`[api/run-llm] Database verification complete`)
      console.log(`[api/run-llm] Verified record exists in database: ${Boolean(verifiedRecord)}`)
    } else {
      console.error(`[api/run-llm] Invalid or missing record ID after insertion. Skipping verification.`)
    }
    
    if (verifiedRecord) {
      console.log(`[api/run-llm] Verification successful - Record exists in database`)
      console.log(`[api/run-llm] Verified record ID: ${verifiedRecord.id}`)
      console.log(`[api/run-llm] Verified record title: ${verifiedRecord.title}`)
      console.log(`[api/run-llm] Verified record walletAddress: ${verifiedRecord.walletAddress}`)
      console.log(`[api/run-llm] Verified record llmService: ${verifiedRecord.llmService}`)
      console.log(`[api/run-llm] Verified record llmModel: ${verifiedRecord.llmModel}`)
      console.log(`[api/run-llm] Verified record transcriptionService: ${verifiedRecord.transcriptionService}`)
      console.log(`[api/run-llm] Verified record transcriptionModel: ${verifiedRecord.transcriptionModel}`)
      console.log(`[api/run-llm] Verified record llmCost: ${verifiedRecord.llmCost}`)
      console.log(`[api/run-llm] Verified record finalCost: ${verifiedRecord.finalCost}`)
      console.log(`[api/run-llm] Verified transcript length: ${verifiedRecord.transcript?.length || 0} characters`)
      console.log(`[api/run-llm] Verified llmOutput length: ${verifiedRecord.llmOutput?.length || 0} characters`)
    } else {
      console.error(`[api/run-llm] VERIFICATION FAILED: Record with ID ${newRecord.id} was not found in database`)
      console.error(`[api/run-llm] This indicates a potential database consistency issue`)
    }
