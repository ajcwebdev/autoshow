...
RUN apt-get update && \
    apt-get install -y build-essential libopenblas-dev pkg-config git wget cmake && \
...
RUN cmake -B build -DGGML_BLAS=1 && cmake --build build -j --config Release
...
RUN apt-get update && \
    apt-get install -y curl ffmpeg libopenblas-dev git wget cmake && \
    rm -rf /var/lib/apt/lists/* /var/cache/apt/archives/*
